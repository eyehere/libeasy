...
void des3_setkey_ex(des3_context *ctx, char * key,long keylen)
{
    ...
    {
        des3_set_2keys( ctx, (BYTE*)key, (BYTE*)key + 8);
    }else if(keylen == 24)
    {
        des3_set_3keys(ctx, (BYTE*)key, (BYTE*)key + 8,(BYTE*)key + 16);
    }
}
void des3_encrypt_ex(des3_context *ctx, char * in,char * out,long len)
{
    int i;
    for (i = 0;i < len;i += 8)
        des3_encrypt(ctx, (BYTE*)in + i, (BYTE*)out + i);
}
void des3_decrypt_ex(des3_context *ctx, char * in,char * out,long len)
{
    int i;
    for (i = 0;i < len;i += 8) {
        des3_decrypt(ctx, (BYTE*)in + i, (BYTE*)out + i);
    }
